window.__define('camunda-hub-bootstrap', [
  './scripts/camunda-hub-ui'
], function() {
  'use strict';

  var camundaHubUi = window.CamundaHubUi;
  ...
  camundaHubUi.exposePackages(requirePackages);
  ...
  requirejs(['globalize'], function(globalize) {
    ...
    requirejs(dependencies, function(angular) {
      // we now loaded the hub and the plugins, great
      // before we start initializing the hub though (and leave the requirejs context),
      // lets see if we should load some custom scripts first

      if (typeof window.camHubConf !== 'undefined' && window.camHubConf.customScripts) {
        var custom = window.camHubConf.customScripts || {};
        ...
        requirejs(custom.deps || [], function() {
          ...
          angular.module('cam.hub.custom', custom.ngDeps);
          ...
          // initialize Hub
          camundaHubUi(pluginDependencies);
        });
      }
      else {
        // for consistency, also create a empty module
        angular.module('cam.hub.custom', []);
...
requirejs(['camunda-hub-bootstrap'], function() {});
